<template>
	<header class="w-full mb-10 text-white bg-slate-900 shadow-sm body-font">
		<div class="container flex flex-col items-start justify-between p-6 mx-auto md:flex-row">
			...
			<nav v-if="user"
				class="flex flex-wrap items-center justify-center pl-24 text-base md:ml-auto md:mr-auto gap-5">
			...
			<div class="items-center h-full" v-if="!user">
				<NuxtLink to="/auth/login" class="mr-5 font-medium hover:text-gray-400">Login</NuxtLink>
				<NuxtLink to="/auth/register" class="mr-5 font-medium hover:text-gray-400">
					Sign Up
				</NuxtLink>
			</div>

			<div class="items-center h-full" v-if="user">
				<NuxtLink to="/user/profile" class="mr-5 font-medium hover:text-gray-400">Profile</NuxtLink>
				<button @click="logout"
