<template>
	<header class="w-full mb-10 text-white bg-slate-900 shadow-sm body-font">
		<div class="container flex flex-col items-center justify-between p-6 mx-auto md:flex-row">
			<a class="flex items-center mb-4 font-medium text-gray-900 title-font md:mb-0">
				<svg class="w-auto h-5 text-white fill-current" viewBox="0 0 202 69" xmlns="http://www.w3.org/2000/svg">
					<path
			...
			<nav v-if="user"
				class="flex flex-wrap items-center justify-center pl-0 md:pl-24 text-base md:ml-auto md:mr-auto gap-5">
				<NuxtLink to="/" class="font-medium hover:text-gray-400">
					Home
				</NuxtLink>
				<NuxtLink to="/smoothie" class="font-medium hover:text-gray-400">
					Smoothies
				</NuxtLink>
			</nav>
			<div v-if="!user" class="flex flex-wrap justify-center gap-2 mt-4 md:mt-0">
				<NuxtLink to="/auth/login" class="mr-0 md:mr-5 font-medium hover:text-gray-400">Login</NuxtLink>
				<NuxtLink to="/auth/register" class="font-medium hover:text-gray-400">
					Sign Up
				</NuxtLink>
			</div>
			<div v-if="user" class="flex flex-wrap justify-center items-center gap-2 mt-4 md:mt-0">
				<NuxtLink to="/user/profile" class="mr-0 md:mr-5 font-medium hover:text-gray-400">Profile</NuxtLink>
				<button @click="logout"
					class="px-4 py-2 text-xs font-bold text-white uppercase transition-all duration-150 bg-rose-500 rounded shadow outline-none active:bg-rose-600 hover:shadow-md focus:outline-none ease">Logout</button>
			</div>
		</div>
	</header>
</template>
